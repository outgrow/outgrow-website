...
class SupportPlans extends Component {
  ...
  handleUpdatePrice = () => {
    ...
    this.setState({
      ...
      yearlyPrice: parseInt(price * 0.9 / 12).toLocaleString("en-US", currency),
  ...
  render() {
    return (
      <div style={{ position: "relative" }}>
        <Head title="Mailchimp Open Commerce (formerly Reaction Commerce) Chat Support | Official Partner | Live Developer Help | out:grow" />
        <Nav />

        <Header
          backgroundImage="/emergency.jpg"
          body={[
            "Do your Mailchimp Open Commerce developers need help with their code? Do you want the peace of mind of always having a sidekick with you in case of emergencies?",
          ...
          {this.state.step === 0 &&
            <Form>
              ...
              <FieldWrapper>
                ...
                <SubLabel>We'll be legally bound to respect this response time within your chosen working hours (see below). This is only helpful for emergency response.</SubLabel>
              ...
              >
                <div>
                  <p>Your total is <Price><span>{this.state.showPriceMonthly ?
                    this.state.monthlyPrice : this.state.yearlyPrice} per month</span></Price></p>

                  {!this.state.showPriceMonthly &&
                    <SavingsIndicator>You are saving {this.state.yearlySavings} per year by paying yearly.</SavingsIndicator>
                  }
                ...
                <div>
                  <HelpPhone
                    isFloating={this.state.isPriceBreakdownFloating}
                  >
                    Need help? Give us a call 24/7 at <a href="tel:+12816884769">+1 (281) OUT-GROW</a>.
                  </HelpPhone>
                </div>
              </PriceBreakdown>
              ...
              <VisibilitySensor onChange={this.togglePriceBreakdownFloating}>
                <ButtonWrapper keepLeft>
                  <Button backgroundColor={green} color={white} onClick={this.handleNextStep}>Sign up</Button>
                </ButtonWrapper>
              </VisibilitySensor>
            </Form>
          }
